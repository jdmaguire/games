...
- Classic script on `chinese-checkers.html` → `{ CELLS, CORNER, TRI, SEATS, LEVELS, targetOf, partnerOf, startBoard, genMoves, applyMove, undoMove, isWin, isDone, winnerAfter, aiPickMove }`. The board is a **flat `number[121]`** (the star, in the engine's fixed cell order): 0 empty or player id 1–6, where id = home corner + 1. Corners are numbered by screen position — 0 bottom (always the human), 1 lower-right, 2 upper-right, 3 top, 4 upper-left, 5 lower-left — `targetOf(p) = (p + 2) % 6` is the opposite corner and `partnerOf(p) = targetOf(p) + 1` is its owner (the team partner in team games; the anti-blocking gift recipient otherwise).
- Geometry exports: `CELLS` = `[{ x, z }]` cube coords (y implied, for layout: `px = √3·(x + z/2)`, `py = 1.5·z`); `CORNER[i]` = corner index or -1 for the central hexagon; `TRI[k]` = the 10 cell indices of corner k; `SEATS` = `{ 2: [1,4], 3: [1,3,5], 4: [1,2,4,5], 6: [1..6] }` (3-player targets are empty corners; 4-player leaves an opposite pair unused). `startBoard(players)` / `isWin(bd, p)` — free-for-all win = target triangle completely full AND ≥1 marble in it is p's (the anti-blocking rule: the owner's parked marbles count as filled for the incomer). `isDone(bd, p)` is the strict all-ten-own test used per-partner in team games. `winnerAfter(bd, mv, p, teams)` → the winner created by p's move or 0: p's own win, the anti-blocking hand-over when p re-fills their own home (free-for-all only), or the strict both-partners-done team win (returns the mover's id; map to the pair with `partnerOf`). The UI calls this after every move — a win can pop up on the *owner's* move, not just the incomer's; see [[game-engine-internals]].
...
- Worker protocol is the checkers one plus a flag: `{ id, board, side, level, ms, teams } → { id, move }`, `move` is `{ from, to, path, gain, score }` or null; `aiPickMove(bd0, side, level, ms, teams)` matches. `teams` (optional, default false) switches the win test to strict-per-partner and makes every mover maximise its team's summed score. Same `askEngine` wrapper + `file://` sync fallback in `js/chinese-checkers.js`. Search internals (max-n, not negamax) in [[game-engine-internals]].
