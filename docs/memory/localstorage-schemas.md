...
- `engine-eval` (js/index.js ~line 41, plus each game's own setup overlay: `#eval-opt` in chess.html/checkers.html, wired in js/chess.js ~line 707 and js/checkers.js ~line 606) — `"1"` or `"0"` (absent = off). "Show the engine's evaluation bar in Chess & Checkers". All three places read/write the same key, so they stay in sync; the bar appears when a game starts. Each game reads it at boot.
