...
- `LEVELS[0..9]` table drives the 10 difficulty levels: `{ label, depth, ms, noise, random }` — depth 1→15, time budget 60→1300ms, `noise` widens the pool of near-best moves picked from at random, `random` is the chance of playing any legal move (0.85 at level 1, 0 from level 4 up) so young kids can win. Iterative deepening caps at the level's `depth`.
